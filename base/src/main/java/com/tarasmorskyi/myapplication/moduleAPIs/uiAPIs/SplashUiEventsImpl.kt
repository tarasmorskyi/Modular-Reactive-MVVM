...
class SplashUiEventsImpl @Inject internal constructor() : SplashUiEvents {
    ...
    override fun startMainScreen(activity: Activity) = activity.startActivity(MainActivity.createIntent(activity))

    override fun startLoginScreen(activity: Activity) = activity.startActivity(LoginActivity.createIntent(activity))
}